...
设计人&作者：[@雪山飞喵ovo](https://space.bilibili.com/651746680)
...
**可以通过语音控制发光眼的灯光** 👉 [bilibili演示视频](https://www.bilibili.com/video/BV1EsbHzmEL4/) 
...
- 发光眼主控板及灯板的PCB设计（使用立创EDA专业版设计）👉 [OSHWHub开源工程](https://oshwhub.com/flyingmeow/ledeyeforfursuit_v5-0_opensource)
...
硬件电路设计方案如下图所示：主控板MCU使用ESP32-WROOM-32E，板上集成CH340C用于程序下载，由ESP32的GPIO控制发光眼灯板上的WS282B灯珠、经MOS管开关电路控制头内风扇开关。主控板上集成的ASRPRO模组使用UART接口与ESP32通信。此外，主控板上还集成了一个MPU6050传感器。![逻辑框图-250725](pic/逻辑框图-250725.png)
...
![主控板原理图](pic/主控板原理图.png)

![主控板PCB设计](pic/主控板PCB设计.png)

![灯板原理图](pic/灯板原理图.png)

![灯板PCB设计](pic/灯板PCB设计.png)
...
蓝牙串口指令交互功能在`src/ble_msg_hdl.cpp`实现，使用了`BluetoothSerial`库。

在`main.cpp`中的`setup()`函数中初始化蓝牙串口：

```
SerialBT.begin("ESP32_飞喵的发光眼设备"); // 启动蓝牙串口并设置蓝牙的名称
```

初始化函数传入的字符串为ESP32蓝牙设备在与其他设备配对时显示的名称，可以根据需要自定义。

`src/ble_msg_hdl.cpp`的实现逻辑参见下图：

![蓝牙串口交互逻辑](pic/蓝牙串口交互逻辑.png)

语音控制功能在`src/voice_msg_hdl.cpp`实现，逻辑更为简单：直接根据ASRPRO串口发送来的4位字符串区分不同指令并继续进行模式切换、参数修改等动作。在使用天问Block对ASRPRO编程时，只使用“配置模式”即可：

![语音识别程序截图](pic/语音识别程序截图.png)

需要注意：ASRPRO目前暂不支持英文字母和单词的语音识别，仅限识别汉字。



## 后续开发ideas

由于时间&精力有限，本项目还有一些没有完善的细节和一些有待尝试的ideas，在这里跟大家分享一下🐱 希望有时间、有兴趣的朋友们可以尝试共同完善一下？欢迎有这方面经验的小伙伴和我交流！😽 ovo 

**【硬件设计方面】**

- 在开发过程中，貌似发现ESP32-WROOM-32模组的IO34和IO35引脚不能像其他GPIO一样使用？例如当前使用IO35的红色LED5就不能正常闪灯，输出电平只有0.33V，并且保持不变？没有找到什么原因qwq
...
- 关于发光眼外壳结构的设计，目前我只是根据我的自建模头骨设计了外壳形状，可以做到和头骨眼眶的无缝贴合，但是对于其他眼眶大小、形状不同的头骨则需要重新建模。可以考虑设计几种更通用的外壳结构？这样在没有头骨模型源文件或使用海绵头骨的情况下也能适用。

**【软件功能方面】**

- 主控板上的MPU6050在当前版本的固件程序上还没有用起来，可以考虑做一些与动作有关的交互？会有意思一些。比如向左摇头控制左眼眨眼、向右控制右眼眨眼等等。
- 在当前版本的硬件设计中，灯板上单独引出了一个光敏电阻，其分压电路在主控板上。最初设计时，想实现跟随环境亮度自适应改变眼睛亮度的功能，但实际应用中发现不如直接语音控制修改亮度更实用。有兴趣的话可以试一试实现这个功能，可能需要尝试不同的光敏电阻型号以及分压电阻阻值。
- 在交互方面，还可以设计一个手机端的上位机。针对ESP23+WS2812灯带的手机端上位机，貌似已经有一些成熟的项目？我目前还没有研究。
- 一个有意思的idea：在兽聚节目表演/夜场/小聚KTV环境下，识别环境中音乐的节拍，并将呼吸灯模式亮度变化的节奏对准环境音乐的节奏。
  - 使用传统信号处理算法  or  训练AI模型？
  - 手机端实现节奏识别，将节奏同步到ESP32  or  直接在ESP32识别节奏？
...
- 本项目软件部分（发光眼主控板固件程序、语音识别程序）遵循[MIT协议](https://mit-license.org/)，允许用于商业用途，不做限制。
- 本项目硬件部分（主控板及灯板的PCB设计、眼眶外壳及固定结构的3D建模设计）遵循[TAPR-OHL协议](https://tapr.org/the-tapr-open-hardware-license/)：允许用于商业用途，<u>但硬件设计文件必须开源，并且在分发硬件时，必须提供全部设计文件和源代码。</u>
- 若将本项目用于商业用途（包括但不限于基于本项目提供有偿的设计和制作服务、售卖二次设计和开发的发光眼成品等），需明确标注使用了本项目设计方案。（如有这方面想法可以联系我，乐意提供技术支持😽）
...
1. 使用手机蓝牙查找发光眼设备并连接、配对。
   - 主控板上电，打开手机蓝牙，可以发现名为`ESP32_飞喵的发光眼设备`的蓝牙设备，点击配对。

![蓝牙串口使用说明1](https://gitee.com/FlyingMeow/mypic/raw/master/%E8%93%9D%E7%89%99%E4%B8%B2%E5%8F%A3%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E1.png)

2. 使用蓝牙串口调试助手APP连接发光眼设备。
   - 打开SSP蓝牙调试助手软件，搜索可用设备、连接。
   - 连接成功后，注意将收发字符串格式改为`utf-8`。
   - 需要注意，如果有多块主控板使用了同一个蓝牙设备名称，在手机上一次与主控板A配对过的情况下开机主控板B，手机蓝牙会自动配对，但ID不一致会导致SSP蓝牙调试助手连接失败。此时返回上一步将设备取消配对、再重新配对即可。

![蓝牙串口使用说明2](pic/蓝牙串口使用说明2.png)

3. 向发光眼设备下发蓝牙串口指令。
   - 可以使用`help`命令查看常用命令。
   - 蓝牙串口可用于更精细的参数设置调试。
   - 在使用语音指令时，执行指令完毕后也会在蓝牙串口输出信息，方便调试。

![蓝牙串口使用说明3](pic/蓝牙串口使用说明3.png)
