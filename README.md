...
由于时间&精力有限，本项目还有一些没有完善的细节和一些有待尝试的ideas，在这里跟大家分享一下🐱 希望有时间、有兴趣的朋友们可以尝试共同完善一下？😽 ovo 

【硬件设计方面】

- 在开发过程中，貌似发现ESP32-WROOM-32模组的IO34和IO35引脚不能像其他GPIO一样使用？例如当前使用IO35的红色LED5就不能正常闪灯，输出电平只有0.33V，并且保持不变？没有找到什么原因qwq
- 使用两只MOS管控制风扇开关的设计不够优雅：电路比较复杂，而且输出由高转低时会有一个较大的延迟，可能是哪里的电容参数设置不当。这里可以采用更简洁的负载开关控制；或者干脆改为一个升压到12V的DCDC电路，直接控制DCDC芯片的使能；或者另采用一个支持PWM控制的4线风扇。
